...
app.MapGet("/db-check", async () => { try { if (string.IsNullOrEmpty(connectionstring)) return Results.BadRequest("Connection string not configured"); var client = new MongoClient(connectionstring); var dblist = await client.ListDatabaseNamesAsync(); return Results.Ok("MongoDb connected successfully"); } catch (Exception ex) { return Results.Problem($"Mongodb connection failed:{ex.Message}"); } });
