    ...
    const fetchCars = async () => {
      try {
        ...
        if (!car || (Array.isArray(car) && car.length === 0)) {
          // treat empty or invalid response as failure and use demo data
          setError(true);
          setCars(mockCars);
        } else {
          setCars(car as any);
        ...
        console.error("Failed to fetch cars, using mock data:", err);
        setError(true);
        setCars(mockCars);
        setLoading(false);
      }
                ...
                : cars.map((car) => (
                    ...
                    >
                      <div className="relative h-48">
                        ...
                        >
                          <Heart
                            className={`h-4 w-4 ${isSaved(car.id) ? "text-red-500" : "text-gray-500"}`}
