...
export default function QuickActions() {
  ...
  const actionItem = ({ name, icon: Icon, href, color }: typeof actions[0]) => (
    <Link href={href} className="w-full p-2 sm:p-3 text-center hover:scale-105">
  ...
  return (
    <>
      <div className="py-6 sm:py-8 px-0 relative z-20 -mt-10 sm:-mt-12">
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-0.5 sm:gap-1 bg-white rounded-lg shadow-md p-3 sm:p-4 mx-auto max-w-6xl">
          {actions.map((action) => (
            <React.Fragment key={action.name}>{actionItem(action)}</React.Fragment>
          ))}
          <button
            onClick={() => setShowMaintenance(true)}
            className="w-full p-2 sm:p-3 text-center hover:scale-105"
          >
            <div className="flex flex-col items-center gap-1.5 sm:gap-2">
